...
\newcommand{\mcpcp}{federated MCP control plane}

\title{Federated MCP Control Plane: Policy-Aware Access to Multi-Backend Tool Servers}
...
We define that missing \emph{control plane} as a \mcpcp{}: a single URL where every request is authenticated, rate-limited, authorized on MCP methods and tool resources, then routed or answered from a federated catalog.
...
We address the problem in Section~\ref{sec:problem} with one \mcpcp{}: a single URL that sits between agents and many MCP backends.
...
\begin{enumerate}[leftmargin=*, itemsep=0.25em]
  ...
  \item \textbf{Open prototype} (repository \code{mcp-gateway}) implements the same lifecycle on a federated \code{/gateway/mcp} endpoint with YAML policy and a cached catalog, so others can run the design rather than infer it from diagrams alone.
...
Brett~\cite{brett2025mcpgateway} surveys simplified, security-oriented MCP gateways for enterprise AI integration (tunneling, intrusion detection, self-hosting).
That line of work complements ours: we emphasize federated \code{tools/list}, deny-override ABAC on tool resources, and an open evaluation artifact rather than a simplified secure tunnel alone.
Our contribution is architectural material for the control-plane layer, not a change to the MCP spec.
...
\midrule
Federated MCP control plane (this work) & Yes & Yes & No & Hook & Threat table \\
...
The repository \code{mcp-gateway} lets others reproduce Section~\ref{sec:design} without standing up OAuth first.
